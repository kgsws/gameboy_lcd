...
// THIS IS A LESS MEMORY EFFICIENT VERSION
// (but it generates timing closer to the original)
#include <string.h>
#include <stdio.h>
#include <unistd.h>
...
static void IRAM_ATTR prepare_line_isr(void *arg)
{
	...
	for(uint32_t i = 0; i < GBLCD_LINE_START / 2; i++)
	{
		*ptr = base;
		ptr++;
	}
	...
	while((void*)ptr < line_wr + GBLCD_LINE_WIDTH * 2)
	{
		*ptr = base;
		ptr++;
	}
	...
	// CPL + CPG
	ptr[(GBLCD_LINE_WIDTH / 2) - 2] = base | 0xC0C0C0C0;

	// CPG
	ptr[(GBLCD_LINE_WIDTH / 2) - 1] = base | 0x80808080;
	ptr[0] = base | 0x00800000;
	ptr[1] = base | 0x80808080;
	ptr[66] |= 0x80808080;
	ptr[120] |= 0x80808080;
	...
	if(lcd_y < GBLCD_HEIGHT)
	{
		ptr[GBLCD_LINE_START / 2] |= 0x00080000;
		ptr[(GBLCD_LINE_START / 2) - 1] |= 0x00000800;
	}
...
void gblcd_output_init(void *framebuffer)
{
	...
	lcd_buff = framebuffer;
	lcd_ptr = framebuffer;

	line = heap_caps_malloc(GBLCD_LINE_WIDTH * 4, MALLOC_CAP_DMA);
	dma = (lldesc_t*)heap_caps_malloc(sizeof(lldesc_t) * 2, MALLOC_CAP_DMA);

	printf("[GB_in] line %p dma %p\n", line, dma);

	memset(line, 0, GBLCD_LINE_WIDTH * 4);

	dma[0].offset = 0;
	dma[0].size = GBLCD_LINE_WIDTH * 2;
	dma[0].length = GBLCD_LINE_WIDTH * 2;
	dma[0].sosf = 0;
	dma[0].eof = 1;
	dma[0].owner = 1;
	dma[0].buf = line;
	...
	dma[1].offset = 0;
	dma[1].size = GBLCD_LINE_WIDTH * 2;
	dma[1].length = GBLCD_LINE_WIDTH * 2;
	dma[1].sosf = 0;
	dma[1].eof = 1;
	dma[1].owner = 1;
	dma[1].buf = line + GBLCD_LINE_WIDTH * 2;
	...
	I2S1.clkm_conf.clkm_div_num = 10;
